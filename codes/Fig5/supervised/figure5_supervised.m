...
for ref=1:ref_num; %reference index, there are 10 references in PC-VQA dataset
    ...
    for time=1:run_time
        ...
        active_kendall_result(time,:)=active_kendall';
        random_kendall_result(time,:)=random_kendall';
       
    end
    mean_kendall_active=mean(active_kendall_result);
    mean_kendall_random=mean(random_kendall_result);
